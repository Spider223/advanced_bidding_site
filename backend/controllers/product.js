...
const startBid = (req, res) => {
  const id = req.params.id;
    Product.findByIdAndUpdate(
      { _id: id },
      { $set: { auctionStarted: true } },
    )
      .then((data) => {
        if (!data) return res.status(200).json({message:"auction could not start", status:400})
...
Product.findByIdAndUpdate(
      ...
      .then((data) => {
        if (!data) return res.status(200).json({message:"bid could not place", status:400})
        return res.status(200).json({message:"bid placed", status:200, product:data})
      }) .catch((error) => {
        return res.status(500).json({message:"something went wrong", status:500})
      });
}
...
module.exports = { addProduct, getProduct, getSingleProduct, socketServer, startBid, placeBid };
